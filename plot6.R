...
## This is 6 of 6 of my responses to the questions posed by the week 4 project for the
...
## Cities of Baltimore and LA, then we can plot by year - Unlike the previous
## questions, I think a line graph with different colors to represent each city
## may be a better solution, but I plan on trying a few different views.

## This question essentially combines everything we've done thus far.

# Step 3 - Create the visualization to best answer the 6th question:

## Step 3 - 1 Prepare the Data by mergine the dataframes, subsetting for motor
## vehicle values, subsetting the 2 cities, then cleaning the output table
...
BaltimoreMobileEmissions <- summarise(group_by(BaltimoreMobileEmissions,year),sum(Emissions))

#### Rename Columns and Convert Year to String
colnames(BaltimoreMobileEmissions) <- c("Year","Emissions")



#### Now that we filtered for Baltimore, we will complete for LA
LAMobileEmissions <- filter(pmCombined,fips=="06037")

#### Now that we have filtered for the FIPS for LA, we can filter to motor vehicle data previosly tagged
LAMobileEmissions <- filter(LAMobileEmissions,mobile==TRUE)

#### Now that we've applied all filters, we can create the summary table for Baltimore City Motor Vehicle Emission Data
LAMobileEmissions <- summarise(group_by(LAMobileEmissions,year),sum(Emissions))
